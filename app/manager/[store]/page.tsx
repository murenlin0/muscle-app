...
export default function ManagerHubPage() {
  ...
  if (bootstrapping) {
    return (
      <PortalShell title="店長後台" subtitle={store.name} variant="admin" size="lg">
  ...
  return (
    ...
    >
      <div className="grid gap-4 sm:grid-cols-2">
        <AdminHubCard
          href={`${managerBase}/import`}
          icon={FileSpreadsheet}
          title="會員 CSV 匯入"
          description="從 Notion 匯出餘額表，更新本店會員"
        />
        <AdminHubCard
          href={`${managerBase}/reports`}
          icon={BarChart3}
          title="本店報表"
          description="日營收、師傅業績與消費紀錄"
        />
        <AdminHubCard
          href={`${managerBase}/team`}
          icon={Users}
          title="人員與權限"
          description="師傅 PIN、店長權限與啟用狀態"
        />
        <AdminHubCard
          href={`${managerBase}/clients`}
          icon={Contact}
          title="客人資料庫"
